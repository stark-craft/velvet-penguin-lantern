...
function DossierModal({ dossier, error, loading, watched, watchable = true, onClose, onRetry, onWatch, onOpenRelated }) {
  const dialogRef = useRef(null);
  const closeRef = useRef(onClose);
  const open = Boolean(dossier || loading || error);
...
export default function VentureLensApp() {
  ...
  const dossierRequestRef = useRef({ token: 0, controller: null });
  ...
  useEffect(() => {
    ...
    const active = navigationRef.current?.querySelector(`[data-workspace="${page}"]`);
    active?.scrollIntoView?.({ behavior: "auto", block: "nearest", inline: "center" });
  }, [location.pathname, page]);
  useEffect(() => {
    if (loading && !payload && !intelligence && !discovery) return;
    const identifier = new URLSearchParams(location.search).get("focus");
    const kind = { radar: "technology", repositories: "repository", research: "paper", models: "model", datasets: "dataset", patents: "patent" }[page];
    const key = kind && identifier ? `${kind}:${identifier}` : "";
    if (!key || focusedArtifactRef.current === key) return;
    openDossier(kind, identifier);
  }, [discovery, intelligence, loading, location.search, page, payload]);
  ...
  useEffect(() => () => dossierRequestRef.current.controller?.abort(), []);
  useEffect(() => {
  ...
  async function openDossier(kind, id) {
    ...
    dossierRequestRef.current.controller?.abort();
    const token = dossierRequestRef.current.token + 1;
    const controller = new AbortController();
    dossierRequestRef.current = { token, controller };
    ...
    try {
      const result = await (loaders[kind] || getTechnologyDossier)(id, { signal: controller.signal });
      if (dossierRequestRef.current.token !== token) return;
      setDossier(result);
      focusedArtifactRef.current = key;
    } catch (requestError) {
      if (requestError?.name === "AbortError" || dossierRequestRef.current.token !== token) return;
      focusedArtifactRef.current = "";
      setDossierError(requestError.message || "This dossier could not be loaded.");
    } finally {
      if (dossierRequestRef.current.token === token) setDossierLoading(false);
    }
  }
  function closeDossier() {
    dossierRequestRef.current.controller?.abort();
    dossierRequestRef.current = { token: dossierRequestRef.current.token + 1, controller: null };
    setDossier(null);
    setDossierError("");
  ...
  const initialLoading = loading && !payload && !intelligence && !discovery;
  const fatalError = !loading && !!error && !payload && !intelligence && !discovery;
  ...
  return (
    <div className="venture-lens">
      ...
      <section className="vl-research-shell-head">
        ...
        <nav aria-label="Research workspaces" className="vl-nav" ref={navigationRef}>
          {visiblePages.map(([id, label, PageIcon]) => <button aria-current={page === id ? "page" : undefined} className={page === id ? "active" : ""} data-workspace={id} key={id} onClick={() => navigate(routeFor(id))} title={label} type="button"><PageIcon aria-hidden="true" size={15} /><span>{label}</span></button>)}
        </nav>
      ...
      <div className={`vl-page vl-page-${page}`}>
        ...
        {page === "overview" && <>
          <section className="vl-hero vl-command-hero">
            <div className="vl-hero-copy">
              <div className="vl-eyebrow"><Sparkles size={14} /> Research decision desk</div>
              <h1>Evidence, organized for the <em>next decision.</em></h1>
              <p>Move from emerging research to implementation evidence without leaving the TechScout workspace.</p>
              <div className="vl-hero-actions"><button onClick={() => navigate("/venturelens/radar")} type="button">Open technology radar <ArrowIcon /></button><button onClick={() => navigate("/venturelens/research")} type="button">Review research</button></div>
            </div>
            <aside className="vl-live-index">
              <header><span>Live evidence index</span><small>{syncLabel(payload?.github?.refreshed_at || payload?.research?.refreshed_at)}</small></header>
              <div className="vl-live-index-score"><strong>{repositories.length + papers.length}</strong><span>implementation and research records ready to inspect</span></div>
              <div className="vl-live-index-lanes"><span><i />{repositories.length} repositories</span><span><i />{papers.length} papers</span><span><i />{intelligence?.radar?.length || 0} radar signals</span></div>
              <div className="vl-live-index-orbits" aria-hidden="true"><i /><i /><i /><b /></div>
            </aside>
          </section>
          <section className="vl-metrics" aria-label="Venture Lens coverage">
            <article><span>Repositories</span><strong>{repositories.length}</strong><small>{githubCategories.length} technology lenses</small></article>
            <article><span>Research papers</span><strong>{papers.length}</strong><small>{researchCategories.length} research lenses</small></article>
            <article><span>Technology radar</span><strong>{intelligence?.radar?.length || 0}</strong><small>scored decision signals</small></article>
            <article><span>Workspace status</span><strong>{loading ? "Loading" : "Ready"}</strong><small>{payload?.github?.status === "live" || payload?.research?.status === "live" ? "Connected to live public sources" : "Responsible cached snapshot"}</small></article>
          </section>
          <section className="vl-section vl-hot-section"><SectionHead number="LIVE" title="Evidence moving now" copy="A concise cross-section of active implementation and research signals." /><div className="vl-hot-layout"><div><header><Code2 size={18} /><span>Implementation signals</span><button onClick={() => navigate("/venturelens/repositories")} type="button">View all <ArrowIcon /></button></header><div className="vl-repo-grid">{repoCards(hotRepositories)}{!hotRepositories.length && <EmptyWorkspace>No repository signals are available.</EmptyWorkspace>}</div></div><div><header><BookOpen size={18} /><span>Research signals</span><button onClick={() => navigate("/venturelens/research")} type="button">View all <ArrowIcon /></button></header><div className="vl-paper-grid">{paperCards(hotPapers)}{!hotPapers.length && <EmptyWorkspace>No research signals are available.</EmptyWorkspace>}</div></div></div></section>
          <section className="vl-section vl-workspace-links"><SectionHead number="DESKS" title="Focused research workspaces" copy="Open the task you need; each keeps its own evidence and interaction model." /><div>{visiblePages.slice(1).map(([id, label, Icon]) => <button key={id} onClick={() => navigate(routeFor(id))} type="button"><Icon size={20} /><strong>{label}</strong><span>Open workspace</span><ArrowIcon /></button>)}</div></section>
        </>}
      ...
      <DossierModal dossier={dossier} error={dossierError} loading={dossierLoading} onClose={closeDossier} onOpenRelated={openDossier} onRetry={() => dossierRequest && openDossier(dossierRequest.kind, dossierRequest.id)} onWatch={() => dossier && toggleWatch(dossier.kind, dossier.id, dossier.label || dossier.full_name || dossier.title)} watchable={["repository", "paper", "technology"].includes(dossier?.kind)} watched={dossierWatched} />
    </div>
  );
}
