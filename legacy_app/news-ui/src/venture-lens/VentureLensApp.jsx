...
function WatchButton({ active, onClick }) {
  return <button aria-label={active ? "Remove from watchlist" : "Add to watchlist"} className={active ? "vl-icon-action active" : "vl-icon-action"} onClick={(event) => { event.stopPropagation(); onClick(); }} title={active ? "Watching" : "Watch this signal"} type="button"><Star aria-hidden="true" fill={active ? "currentColor" : "none"} size={15} /></button>;
}
...
function RepoCard({ repository, watched, compared, onOpen, onWatch, onCompare }) {
  ...
  return (
    <article className="vl-repo-card interactive" onClick={() => onOpen("repository", repository.id)}>
      <header><span className={`vl-repo-mark is-${repository.category}`}>{String(repository.name || "R").slice(0, 2).toUpperCase()}</span><div><span>{repository.owner}</span><h3>{repository.name}</h3></div><div className="vl-card-tools"><WatchButton active={watched} onClick={onWatch} /><CompareButton active={compared} onClick={onCompare} /></div></header>
...
function PaperCard({ paper, watched, compared, onOpen, onWatch, onCompare }) {
  return (
    <article className="vl-paper-card interactive" onClick={() => onOpen("paper", paper.id)}>
      ...
      <div className="vl-paper-copy"><div className="vl-paper-meta"><span>{paper.category?.replaceAll("-", " ")}</span><div className="vl-card-tools"><WatchButton active={watched} onClick={onWatch} /><CompareButton active={compared} onClick={onCompare} /></div></div><h3>{paper.title}</h3><p>{paper.summary}</p><div className="vl-paper-authors">{(paper.authors || []).slice(0, 3).join(" · ") || "Research collective"}</div><div className="vl-paper-actions"><button type="button">Open dossier</button><a href={paper.url} onClick={(event) => event.stopPropagation()} rel="noreferrer" target="_blank"><BookOpen size={13} /> arXiv <ArrowIcon /></a></div></div>
    </article>
  );
}

function CategoryRail({ categories, active, onChange }) {
  return <div className="vl-category-rail" role="tablist" aria-label="Topic categories"><button className={active === "all" ? "active" : ""} onClick={() => onChange("all")} type="button">All signals</button>{categories.map((category) => <button className={active === category.id ? "active" : ""} key={category.id} onClick={() => onChange(category.id)} type="button"><i className={`is-${category.id}`} />{category.label}</button>)}</div>;
}
...
function metricValue(item, definition) {
  ...
  if (definition.format === "number") return compactNumber.format(Number(raw));
...
export default function VentureLensApp() {
  ...
  async function loadWorkspace() {
    ...
    try { const [overview, derived] = await Promise.all([getVentureOverview(), getVentureIntelligence()]); setPayload(overview); setIntelligence(derived); }
  ...
  const filteredRepositories = useMemo(() => repositories.filter((item) => (repoCategory === "all" || item.category === repoCategory) && (!normalizedQuery || [item.name, item.full_name, item.description, ...(item.topics || [])].join(" ").toLowerCase().includes(normalizedQuery))), [repositories, repoCategory, normalizedQuery]);
  const filteredPapers = useMemo(() => papers.filter((item) => (paperCategory === "all" || item.category === paperCategory) && (!normalizedQuery || [item.title, item.summary, ...(item.authors || [])].join(" ").toLowerCase().includes(normalizedQuery))), [papers, paperCategory, normalizedQuery]);
  ...
  async function toggleWatch(kind, id, label) { try { const result = await toggleVentureWatchlist({ kind, id, label }); const derived = await getVentureIntelligence(); setIntelligence({ ...derived, watchlist: result.items }); } catch (requestError) { setError(requestError.message); } }
  ...
  async function readNotifications() { const items = await markVentureNotificationsRead(); setIntelligence((current) => ({ ...current, notifications: items.items || items })); }
  const dossierWatched = dossier ? watchedKeys.has(referenceKey(dossier.kind, dossier.id)) : false;

  const repoCards = (items) => items.map((repository) => <RepoCard compared={comparedKeys.has(referenceKey("repository", repository.id))} key={`${repository.category}-${repository.id}`} onCompare={() => toggleCompare("repository", repository.id, repository.full_name || repository.name)} onOpen={openDossier} onWatch={() => toggleWatch("repository", repository.id, repository.full_name || repository.name)} repository={repository} watched={watchedKeys.has(referenceKey("repository", repository.id))} />);
  const paperCards = (items) => items.map((paper) => <PaperCard compared={comparedKeys.has(referenceKey("paper", paper.id))} key={`${paper.category}-${paper.id}`} onCompare={() => toggleCompare("paper", paper.id, paper.title)} onOpen={openDossier} onWatch={() => toggleWatch("paper", paper.id, paper.title)} paper={paper} watched={watchedKeys.has(referenceKey("paper", paper.id))} />);
