      ...
      {safeItems.length>1 && (
        <>
          ...
          <div className="tsan-featured-dots" role="tablist" aria-label="Featured stories">
            {safeItems.map((_,i)=>(
              <button key={i} aria-label={`Show featured story ${i+1}`} aria-selected={i===safeIndex} className={`tsan-dot${i===safeIndex?' is-active':''}`} onClick={()=> setIndex(i)} role="tab" type="button" />
            ))}
          </div>
        </>
      )}
