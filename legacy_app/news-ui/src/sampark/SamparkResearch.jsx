...
export default function SamparkResearch() {
  ...
  // sync url query to input when navigation changes
  ...
  const executeSearch = async ({ q, from, to, sites, append=false }) => {
    ...
    try {
      ...
      if (!append) {
        // push to url for persistence without reload
        const next = new URLSearchParams(location.search);
        if (clean) next.set('q', clean); else next.delete('q');
        const qs = next.toString();
        navigate(`/research${qs ? `?${qs}` : ''}`, { replace: true });
      }
    } catch (e) {
  ...
  // auto search when urlQuery present on mount / navigation
  useEffect(()=>{
  ...
  return (
    <div className="sampark-research-page">
      ...
      <header className="sampark-research-header">
        <div>
          <span className="sampark-research-kicker">Research · extracted intelligence</span>
          <h1>Investigate the retained archive</h1>
          <p>Full-text search across scheduler-extracted briefing files — never the live crawler, never the public web.</p>
        </div>
      ...
      <form className="sampark-research-search" onSubmit={handleSubmit} role="search">
        <div className="sampark-research-query">
          ...
          <input aria-label="Research query" placeholder="Company, product, technology, market, or exact phrase" value={query} onChange={(e)=>setQuery(e.target.value)} type="search" />
      ...
      <div className="sampark-research-status" aria-live="polite">
        ...
        {!loading && !error && !items.length && !query.trim() && <span>Enter a query above to surface ranked archive signals. Results appear grouped by date, newest first.</span>}
