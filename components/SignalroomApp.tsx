...
import { AdvancedFilterDrawer, ArticleDetails, ExportModal, FeedbackModal, HistorySnapshotDrawer, NotificationsDrawer, ProfileModal, ScanModal, SourceModal } from "@/components/Overlays";
import { EmptyState, GlobalTooltip, Icon, ToastRegion } from "@/components/ui";
import { downloadExport, mapRawArticle, mapSource, signalroomApi, type ScanJob, type VocResponse } from "@/lib/signalroom-client";
...
export default function SignalroomApp({ initialViewer, capabilities: initialCapabilities }: { initialViewer: ViewerProfile; capabilities: AccessCapabilities; initialTelemetry?: null }) {
  ...
  const applySets = useCallback((desk: DeskProfile, records: Record<string, Awaited<ReturnType<typeof signalroomApi.worklist>>>) => {
    ...
    const nextWorkflow = workflowStates.flatMap((state) => (records[state]?.items ?? []).filter((item) => !seen.has(item.article.id) && seen.add(item.article.id)).map((item) => { const article = mapRawArticle(item.article); return { id: `${desk}:${article.id}`, articleId: article.id, title: article.headline, category: article.category, owner: viewer.displayName, team: article.team.replace(" Team", ""), priority: article.priority, status: statusFromDisposition(item.disposition), created: article.date, due: "—", notes: "Backend editorial worklist", attachments: item.article.sources.length, exported: false }; }));
  ...
  const loadDesk = useCallback(async (requested?: DeskProfile) => {
    ...
    try {
      ...
      setViewer((current) => ({ ...current, id: me.actor_id, accountEmail: me.identity ?? current.accountEmail, displayName: me.preferences?.display_name?.trim() || current.displayName, contactEmail: me.preferences?.contact_email ?? current.contactEmail, roleLabel: me.capabilities.includes("analytics") ? "Intelligence lead" : "Intelligence analyst" }));
      const states = ["selected", "saved", "under_review", "approved", "interesting", "not_interested", "hidden"];
      const [feed, sourceRows, historyRows, ...worklists] = await Promise.all([signalroomApi.feed(desk), signalroomApi.sources(desk), signalroomApi.briefings(desk), ...states.map((state) => signalroomApi.worklist(desk, state))]);
      ...
      if (me.capabilities.includes("gatekeeper_review")) setGatekeeper(await signalroomApi.gatekeeper(desk)); else setGatekeeper(null);
  ...
  const navigate = (id: string) => { if (id === "analytics" && !capabilities.canViewAnalytics) return addToast("Analytics is restricted", "warning"); if ((id === "gatekeeper" || id === "dropped") && !capabilities.canReviewGatekeeper) return addToast("Gatekeeper review is restricted", "warning"); setActivePage(id); setMobileNavOpen(false); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  const setArticleStatus = async (id: string, nextStatus: string) => {
    try {
      if (nextStatus === "New") {
        ...
        await signalroomApi.action(profile, id, "approve");
      } else return;
    ...
    } catch (reason) { showError(reason, "Article status could not be updated"); }
  };
  ...
  const saveViewer = async (next: ViewerProfile) => { try { const savedPreference = await signalroomApi.savePreferences({ display_name: next.displayName, contact_email: next.contactEmail || null }); setViewer({ ...next, displayName: savedPreference.display_name, contactEmail: savedPreference.contact_email ?? "" }); window.localStorage.removeItem(`signalroom-profile-draft:${next.id}`); addToast(`Saved. Signalroom will call you ${savedPreference.display_name}.`); } catch (reason) { try { window.localStorage.setItem(`signalroom-profile-draft:${next.id}`, JSON.stringify({ displayName: next.displayName, contactEmail: next.contactEmail })); } catch { /* draft is best effort */ } showError(reason, "Identity was not saved; the draft remains on this device"); throw reason; } };
  ...
  const renderPage = () => {
    ...
    if (activePage === "search") return <SearchView articles={searchArticles} briefingIds={briefingArticleIds} historyIds={historyIds} workflowIds={workflowIds} savedIds={saved} onSearch={(searchQuery, resultCount, scope) => track("search", { query_length: searchQuery.length, result_count: resultCount, scope })} onOpen={(article) => void openArticle(article)} selectedIds={selectedIds} onToggleSelected={toggleSelected} />;
    ...
    if (activePage === "gatekeeper" && capabilities.canReviewGatekeeper) return <GatekeeperView audit={gatekeeper} onDropped={() => navigate("dropped")} />;
    if (activePage === "dropped" && capabilities.canReviewGatekeeper) return <DroppedView audit={gatekeeper} onRestore={(id) => void setArticleStatus(id, "Under Review")} onHide={(id) => void performAction(id, "hide", "Article hidden")} onWorkflow={(id) => void setArticleStatus(id, "Under Review")} />;
  ...
  return <div className={`signalroom-app ${sidebarCollapsed ? "nav-collapsed" : ""} ${mobileNavOpen ? "mobile-nav-open" : ""} ${selectedIds.size ? "has-selection" : ""}`} data-profile={profile} data-theme={theme}>
    ...
    <Sidebar active={activePage} collapsed={sidebarCollapsed} onNavigate={navigate} onToggle={() => setSidebarCollapsed(!sidebarCollapsed)} profile={profile} canSwitchProfile={capabilities.canSwitchDeskProfile} canViewAnalytics={capabilities.canViewAnalytics} canReviewGatekeeper={capabilities.canReviewGatekeeper} onProfileChange={changeProfile} onFeedback={() => setFeedbackOpen(true)} sourceCount={sources.filter((source) => source.enabled).length} lastBriefingAt={lastBriefingAt} droppedCount={gatekeeper?.counters.articles.dropped ?? 0} />
    {mobileNavOpen && <button className="mobile-scrim" onClick={() => setMobileNavOpen(false)} aria-label="Close navigation" />}
    <Header pageLabel={pageLabels[activePage] ?? "Signalroom"} onScan={() => setScanOpen(true)} onNotifications={() => setNotificationsOpen(true)} onSearch={() => setActivePage("search")} onMenu={() => { setSidebarCollapsed(false); setMobileNavOpen((open) => !open); }} menuOpen={mobileNavOpen} notificationsCount={0} scanRunning={scanRunning} profile={profile} theme={theme} onThemeChange={() => setTheme(theme === "light" ? "dark" : "light")} viewer={viewer} profileOpen={profileOpen} onProfile={() => setProfileOpen(true)} lastBriefingAt={lastBriefingAt} />
    ...
    {profileOpen && <ProfileModal open onClose={() => setProfileOpen(false)} viewer={viewer} accessLabel={capabilities.accessLabel} onSave={saveViewer} />}
